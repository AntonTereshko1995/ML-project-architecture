...
from src.carpricer.train import evaluator
from xgboost.sklearn import XGBRegressor
...
class PatchedXGBRegressor(XGBRegressor):
    def __sklearn_tags__(self):
        def __sklearn_tags__(self):
            """
            Define Scikit-learn tags for this estimator.
            Tags control estimator behavior in various Scikit-learn workflows.
            """
            return {
                "multioutput": False,         # Does not support multioutput
                "requires_y": True,           # Requires a target variable (y)
                "non_deterministic": False,   # Produces deterministic output
                "poor_score": False           # Produces reliable scores
            }
